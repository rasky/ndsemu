...
func (g *Generator) writeOpSwp(op uint32) {
	...
	if byt {
		fmt.Fprintf(g, "cpu.Regs[rdx] = reg(cpu.Read8(rn))\n")
		fmt.Fprintf(g, "cpu.Write8(rn, uint8(rm))\n")
	} else {
		fmt.Fprintf(g, "res := reg(cpu.Read32(rn))\n")
		...
		fmt.Fprintf(g, "cpu.Write32(rn, rm)\n")
	}
	g.writeCycles(1)
...
func (g *Generator) writeOpBx(op uint32) {
	...
	fmt.Fprintf(g, "if rn&1 != 0 { cpu.Cpsr.SetT(true,cpu); rn &^= 1 } else { rn &^= 3 }\n")
	if link {
...
func (g *Generator) writeOpBranch(op uint32) {
	...
	fmt.Fprintf(g, "  cpu.Cpsr.SetT(true,cpu)\n")
...
func (g *Generator) writeOpMemory(op uint32) {
	...
	if load {
		if byt {
			fmt.Fprintf(g, "res := uint32(cpu.Read8(rn))\n")
			name = "ldrb"
		} else {
			fmt.Fprintf(g, "res := cpu.Read32(rn)\n")
			// LDR and SWP only do a bitwise rotation in case of misaligned address
			fmt.Fprintf(g, "if rn&3!=0 { rot := (rn&3)*8; res = (res>>rot) | (res << (32-rot)) }\n")
			name = "ldr"
		}
		fmt.Fprintf(g, "cpu.Regs[rdx] = reg(res)\n")
		fmt.Fprintf(g, "if rdx == 15 {\n")
		fmt.Fprintf(g, "  if res&1!=0 { cpu.Cpsr.SetT(true,cpu); res&^=1 } else { res&^=3  }\n")
		g.writeBranch("reg(res)", "BranchJump")
		fmt.Fprintf(g, "}\n")
	} else {
		fmt.Fprintf(g, "rd := cpu.Regs[rdx]\n")
		if byt {
			fmt.Fprintf(g, "cpu.Write8(rn, uint8(rd))\n")
			name = "strb"
		} else {
			fmt.Fprintf(g, "cpu.Write32(rn, uint32(rd))\n")
			name = "str"
		}
	}
...
func (g *Generator) writeOpHalfWord(op uint32) {
	...
	case 1:
		if load {
			fmt.Fprintf(g, "// LDRH\n")
			name = "ldrh"
			fmt.Fprintf(g, "res := cpu.Read16(rn)\n")
			...
			fmt.Fprintf(g, "// STRH\n")
			name = "strh"
			fmt.Fprintf(g, "cpu.Write16(rn, uint16(cpu.Regs[rdx]))\n")
		}
	case 2:
		if load {
			fmt.Fprintf(g, "// LDRSB\n")
			name = "ldrsb"
			fmt.Fprintf(g, "data := int32(int8(cpu.Read8(rn)))\n")
			...
			fmt.Fprintf(g, "cpu.Regs[rdx+1] = reg(cpu.Read32(rn+4))\n")
	...
	case 3:
		if load {
			...
			fmt.Fprintf(g, "cpu.Write32(rn+4, uint32(cpu.Regs[rdx+1]))\n")
		}
	}
...
func (g *Generator) writeOpBlock(op uint32) {
	...
	if load {
		fmt.Fprintf(g, "val := reg(cpu.Read32(rn))\n")
		...
		fmt.Fprintf(g, "  if newpc&1 != 0 {cpu.Cpsr.SetT(true,cpu); newpc &^= 1} else {newpc &^= 3}\n")
		g.writeBranch("   newpc", "BranchJump")
		fmt.Fprintf(g, "}\n")
	} else {
		fmt.Fprintf(g, "var val uint32\n")
		fmt.Fprintf(g, "val = uint32(cpu.Regs[i])\n")
		fmt.Fprintf(g, "cpu.Write32(rn, val)\n")
	}
	if !pre {
		fmt.Fprintf(g, "rn += 4\n")
	}
